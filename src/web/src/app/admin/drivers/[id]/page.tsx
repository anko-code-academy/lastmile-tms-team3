...
export default function DriverDetailPage() {
  ...
  const [avail, setAvail] = useState<DriverAvailability>({
    schedule: [],
    daysOff: [],
  });
  ...
  useEffect(() => {
    Promise.all([
      getDriverAction(id),
      getDepotsAction(),
      getUsersAction(undefined, "DRIVER"),
    ])
      .then(([d, deps, usr]) => {
        if (!d) {
  ...
  function handleCancelAvail() {
    if (driver) {
      ...
      setDaysOffForm(
        driver.availability.daysOff.map((x) => ({
          date: x.date,
          isPaid: x.isPaid,
          reason: x.reason ?? "",
        })),
  ...
  async function handleSaveAvail() {
    ...
    const schedule: DriverScheduleInputItem[] = DAYS.filter(
      (d) => scheduleForm[d].enabled,
    ).map((d) => ({
      dayOfWeek: d,
  ...
  async function handleToggleStatus(newStatus: boolean) {
    ...
    if (!result.error) {
      setDriver((d) => (d ? { ...d, isActive: newStatus } : d));
      await queryClient.invalidateQueries({ queryKey: ["drivers"] });
    }
  }
          ...
          <div style={{ padding: "2rem", maxWidth: "700px", margin: "0 auto" }}>
            ...
            <div style={{ marginBottom: "2rem" }}>
              ...
              >
                ...
                <span
                  style={{
                    ...
                    padding: ".2rem .5rem",
                    borderRadius: 4,
                    border: `1px solid ${driver.isActive ? "rgba(34,197,94,.3)" : "rgba(239,68,68,.3)"}`,
                    color: driver.isActive ? S.green : S.red,
                    textTransform: "uppercase",
                  }}
                >
              ...
              >
                ...
                {driver.depot?.name ?? "No depot assigned"}
              </p>
            </div>
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 6,
                      cursor:
                        driver.isActive === value || statusLoading
                          ? "default"
                          : "pointer",
                      opacity: statusLoading ? 0.5 : 1,
                      border: `1px solid ${driver.isActive === value ? (value ? "rgba(34,197,94,.4)" : "rgba(239,68,68,.4)") : S.border}`,
                      background:
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    padding: ".45rem .9rem",
                    borderRadius: 6,
                    cursor: userLinking ? "not-allowed" : "pointer",
                    opacity: userLinking ? 0.5 : 1,
              ...
              {driver.userId && (
                ...
                >
                  Linked ID:{" "}
                  <span style={{ color: S.dim }}>{driver.userId}</span>
                </p>
              )}
              {userLinkError && (
                <p
              ...
              <form onSubmit={handleSave}>
                ...
                >
                  <div>
                    ...
                    <input
                      id="firstName"
                      className={editing ? "tm-input" : undefined}
                      disabled={!editing}
                ...
                >
                  ...
                  <div>
                    <TmLabel htmlFor="licenseExpiryDate">
                      License Expiry
                    </TmLabel>
                    <input
            ...
            >
              ...
              >
                {DAYS.map((day) => {
                  ...
                  const liveRow = avail.schedule.find(
                    (s) => s.dayOfWeek === day,
                  );
                  return (
                    <div
                        ...
                        <>
                          <label
                            style={{
                              ...
                              fontFamily: S.mono,
                              fontSize: ".8rem",
                              color: row.enabled ? S.text : S.dim,
                              cursor: "pointer",
                            }}
                          >
                            <input
                        ...
                        <>
                          ...
                          <span
                            style={{
                              fontFamily: S.mono,
                              fontSize: ".8rem",
                              color: liveRow ? S.muted : S.dim,
                            }}
                          >
                            {liveRow
                              ? `${liveRow.startTime?.slice(0, 5) ?? "—"} – ${liveRow.endTime?.slice(0, 5) ?? "—"}`
                              : "Off"}
                          </span>
                        </>
                      )}
              ...
              {availEditing ? (
                ...
                >
                  ...
                  {daysOffForm.map((d, i) => (
                    <div
                      ...
                      style={{
                        display: "grid",
                        gridTemplateColumns: "150px 90px 1fr 32px",
                        gap: ".5rem",
                        alignItems: "center",
                      }}
                    >
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setDaysOffForm((f) => f.filter((_, j) => j !== i))
                        }
                        style={{
                          fontFamily: S.mono,
                          fontSize: "12px",
                ...
                <div
                  style={{
                    ...
                    flexDirection: "column",
                    gap: ".35rem",
                    marginBottom: availEditing ? "1.25rem" : 0,
                  }}
                >
                  {avail.daysOff.length === 0 ? (
                    ...
                    avail.daysOff.map((d, i) => (
                      ...
                      >
                        ...
                        <span style={{ color: d.isPaid ? S.green : S.muted }}>
                          {d.isPaid ? "Paid" : "Unpaid"}
                        </span>
                        {d.reason && (
                          <span style={{ color: S.dim }}>{d.reason}</span>
                        )}
                      </div>
                    ))
                  )}
